...
- **Forms:** code-first forms from `src/SproutForms.Site/Code/`, registered on startup in this environment only.
  ...
  - Regression forms, one per awkward case:
    ...
    - `aiTestFormType`: uses the `aiTestQuiz` form type (`Code/AiTestQuizFormType.cs`, registered in `Program.cs` with its backoffice descriptor). The quiz has a pass mark setting, extends radio fields with a correct answer and points, and rejects file fields. The stored definition should start with `"Type":{"TypeAlias":"aiTestQuiz","Settings":{"PassMark":2}}`, and the `coffee` field should have `"Extension":{"FormTypeAlias":"aiTestQuiz","Settings":{"CorrectAnswer":"espresso","Points":2}}`. Adding a file field, or an extension on a field type the quiz doesn't extend, makes startup fail with the validator's message.
