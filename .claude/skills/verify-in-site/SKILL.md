...
- **Forms:** code-first forms from `src/SproutForms.Site/Code/`, registered on startup in this environment only.
  ...
  - Regression forms, one per awkward case:
    ...
    - `aiTestMultiPage`: four pages. Page 1 ("About you", Next reads "Continue") has a required `name` and `email`, and a `delivery` radio (`pickup` or `home`). Page 2 ("Your order", Previous reads "Back") has a required `orderCode` matching `^[A-Z]{3}-[0-9]{3}$` and an optional `receipt` upload. Page 3 ("Delivery address") only shows when `delivery` is `home`, and has a required `address`; otherwise it's skipped, hidden from the progress list, and its empty `address` doesn't block the submit. Page 4 has no title, so the progress list shows "Step 4", and has a required `message`. Submit reads "Send order". Only the current page shows (`[data-sf-page]` without `hidden`), and every page change raises `sproutforms:pagechange` on the form. Next posts the page to `/api/forms/{id}/pages/{index}/validate` (400 with `errors`, or 200), so an invalid email on page 1 is a server-only error; watch it with `read_network_requests` and `urlPattern: "/validate"`.
