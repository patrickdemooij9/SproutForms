...
public interface IRepositoryCachePolicy<TEntity, TId> where TEntity : class
	{
		/// <summary>
		...
		TEntity? Get(TId? id, Func<TId?, TEntity?> performGet);

        TEntity[] GetByProperty<TProperty>(
		...
		void Delete(TId id, Action<TId> persistDeleted);
